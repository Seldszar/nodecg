...
function getPanelWorkspaces(panel) {
	const workspaces = [];

	if (panel.workspace) {
		workspaces.push(panel.workspaces);
	}

	if (!(panel.fullbleed || panel.dialog) && panel.workspaces) {
		workspaces.push(...panel.workspaces);
	}
...
module.exports = function (dashboardDir, bundle) {
	...
	bundle.dashboardPanels.forEach((panel, index) => {
		...
		panel.workspaces = getPanelWorkspaces(panel);
		panel.html = html;

		if (!panel.dialog && panel.workspaces.some(w => w.startsWith('__nodecg'))) {
