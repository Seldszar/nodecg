...
module.exports.start = function () {
	...
	bundleManager.init(process.env.NODECG_ROOT, require('../../package.json').version, config, Logger).then(() => {
		...
		if (config.developer) {
			...
			// The browser API only exists in the "build" folder, so when we're in dev mode we have to re-route
			// these two specific files to always go to the "build" folder, never the "src" folder.
		...
		// If in developer mode, serve /src as static files. Else, serve /build as static files.
		app.use('/', express.static(config.developer ? SRC_PATH : BUILD_PATH));

		// This route is a bit weird. It means that we're completely ignoring the /build/bower_components dir
		// and not using it for anything. But, that dir only contains the Polymer elements that get moved
		// by polymer-build, and not _every_ bower component. So, we stick with just serving the original
		// bower_components folder at all times just so everything works. There's room to improve this.
		...
		// Serve bundle-specific bower components.
		...
		// 1) Prevents the dashboard/graphics from being opened before everything has finished loading
		// 2) Prevents dashboard/graphics from re-declaring replicants on reconnect before extensions have had a chance
