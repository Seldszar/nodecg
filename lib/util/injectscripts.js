...
module.exports = function (pathOrHtml, resourceType, opts, cb) {
	...
	function inject(err, html) {
		...
		const firstScript = document.querySelector('link[rel="import"], script');

		if (firstStyle) {
			firstStyle.insertAdjacentHTML('beforebegin', styles.join('\n'));
		} else {
			document.head.insertAdjacentHTML('beforeend', styles.join('\n'));
		}

		if (firstScript) {
			firstScript.insertAdjacentHTML('beforebegin', scripts.join('\n'));
		} else {
			document.body.insertAdjacentHTML('beforeend', scripts.join('\n'));
		}
