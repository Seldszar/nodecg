...
server.on('started', () => {
	io.sockets.on('connection', socket => {
		socket.on('replicant:declare', async (data, cb = function () {}) => {
			log.replicants('received replicant:declare', data);

			try {
				const replicant = await findOrDeclare(data.name, data.namespace, data.opts);
		...
		socket.on('replicant:proposeAssignment', async (data, cb = function () {}) => {
			log.replicants('received replicant:proposeAssignment', data);
			const serverReplicant = await findOrDeclare(data.name, data.namespace, data.opts);

			if (data.schemaSum !== serverReplicant.schemaSum) {
				cb({
		...
		socket.on('replicant:proposeOperations', async (data, cb = function () {}) => {
 ...
 * @returns {object}
 */
async function declare(name, namespace, opts) {
	return require('./replicant').load(name, namespace, opts);
}
 ...
 * Defaults to `nodecg/bundles/${bundleName}/schemas/${replicantName}.json`.
 */
async function findOrDeclare(name, namespace, opts) {
...
function saveReplicant(replicant) {
	...
	process.nextTick(async () => {
		...
		const value = encoder.encode(replicant.value);

		try {
			const [doc, created] = await models.Replicant.findCreateFind({
				where: {
					name: `${replicant.namespace}:${replicant.name}`
				},
				defaults: {
					value
				}
