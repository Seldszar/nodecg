...
class Replicant extends EventEmitter {
	static async load(name, namespace, opts = {}) {
		return (new Replicant(name, namespace, opts)).waitUntilLoad();
	}
	...
	constructor(name, namespace, opts = {}) {
		...
		if (this.schema) {
			if (typeof this.opts.defaultValue === 'undefined') {
				this.opts.defaultValue = schemaDefaults(this.schema);
			}

			if (typeof this.opts.defaultValue !== 'undefined') {
				this.validate(this.opts.defaultValue);
			}
		}

		this.value = shared._proxyRecursive(this, this.opts.defaultValue, '/');
		this.log.replicants('Declared "%s" in namespace "%s" with defaultValue:\n', this.name, this.namespace, this.opts.defaultValue);
	...
	waitUntilLoad() {
		return this._loadDeferrable.promise;
	}
	...
	async _load() {
		const result = await models.Replicant.findOne({
			where: {
				name: `${this.namespace}:${this.name}`
			}
		});

		const existingValue = encoder.decode(result && result.get('value'));

		if (this.opts.persistent && typeof existingValue !== 'undefined' && existingValue !== null) {
			if (this.validate(existingValue, {throwOnInvalid: false})) {
				this.value = shared._proxyRecursive(this, existingValue, '/');
				this.log.replicants('Loaded a persisted value from database:', existingValue);
			} else {
		...
		this._loaded = true;
		this._loadDeferrable.resolve(this);
	}
}
