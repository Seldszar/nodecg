...
module.exports = async () => {
	const app = express();
	const collectionsRep = await Replicant.load('collections', '_assets', {
		defaultValue: [],
		persistent: false
	});
	...
	await pMap(collections, async ({name, categories}) => {
		...
		await pMap(categories, async category => {
			...
			const assetRep = await Replicant.load(`assets:${category.name}`, name, {
				defaultValue: [],
				persistent: false
			});

			replicantsByNamespace[name][category.name] = assetRep;
