...
module.exports = async () => {
	...
	Replicant.load('volume:master', '_sounds', {defaultValue: 100});

	await pMap(bundles.all(), async bundle => {
		...
		if (bundle.soundCues.length > 0) {
			...
			const cuesRep = await Replicant.load('soundCues', bundle.name, {
			...
			Replicant.load(`volume:${bundle.name}`, '_sounds', {defaultValue: 100});
		}
	});
