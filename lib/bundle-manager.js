...
module.exports.init = function (bundlesPath, cfgPath, nodecgVersion, nodecgConfig, Logger, isEmbedInstance) {
	if (initialized) {
	...
	if (isEmbedInstance) {
		const bundlePath = process.env.NODECG_ROOT;
		const {name: bundleFolderName} = path.parse(bundlePath);

		loadBundle({bundleFolderName, bundlePath, cfgPath, nodecgVersion, nodecgConfig});
	} else {
...
function loadBundle({bundleFolderName, bundlePath, cfgPath, nodecgVersion, nodecgConfig}) {
